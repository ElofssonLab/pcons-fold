...
\section{Results:} Using the same 12 proteins as in the original
publication of EVfold the TM-score of the top ranked models are
improved by on average 33\%.  We find that on average the quality is
improved with about 15\% when the improved contacts from plmDCA are
used and another 16\% when PconsC is used. Using Rosetta instead of
CNS does not significantly improve global model accuracy. However, the
chemistry of models generated with Rosetta is improved.
...
with $>$95\% confidence if its absolute value is larger than 0.02.
...
without known structure that are suitable for coevolution-based
...
even further enhanced by focussing future work on long-range
contacts.}
