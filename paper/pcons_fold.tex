...
\section{Motivation:}
Recent progress in protein residue-residue contact prediction has led to increasingly accurate contact maps. Predicted contacts are used to facilitate protein structure prediction. However, until now it remained unclear wether there is a direct connection between improvements in contact predictions and resulting model quality.
Here we present PconsFold, a fully automated pipeline for ab-initio protein structure prediction based on evolutionary information. PconsFold is based on PconsC contact prediction and uses a Rosetta folding protocol. Due to its modularity, the contact prediction tool can be easily exchanged. We also use PSICOV and plmDCA contacts to show that improved quality of predicted contacts increases quality and native-likeliness of predicted models.
\section{Results:}
PconsFold provides an average increase in TM-score of maximal 17\% compared to EVfold. PconsFold performs best on average and shows significant increase in TM-score in 7 targets on a test dataset of 15 proteins. Using contacts from PconsC instead of PSICOV or plmDCA increases average model quality, measured in TM-score, by 10\%. Improved contact maps are thus a major factor to improve model quality in ab-initio structure predictions.
\section{Availability:}
The source code of PconsFold is available on GitHub at \url{https://www.github.com/ElofssonLab/pcons-fold}
...
to be modeled less accurate. \\\indent 
Proteins belonging to the {\it  mainly alpha} CATH fold class seem to be easier to fold than
...
We assume Rosetta scoring performs best, because predicted contacts are part of the energy function. Models that satisfy predicted contacts most optimally are assigned low energies and thus ranked on top. \\\indent
