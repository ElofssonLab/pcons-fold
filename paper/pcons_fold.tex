...
\section{Results:} \red{In a small benchmark of 15 proteins} we
show that the TM-scores of top ranked models are improved by on average
33\% \red{using PconsFold compared to the original version of EVfold}.
In a larger benchmark we find that the quality is improved with
15-30\% \red{when using PconsC in comparison to earlier contact
...
at \url{https://www.github.com/ElofssonLab/PconsFold} under the MIT
...
relative to sequence length. \red{ Consider $f$ is set to 1.0.
...
annealing. The CNS based \red{folding} protocol used in EVfold is approximately
...
\red{Accurate stereochemistry becomes important when the predicted models
...
expected decline in average model quality\red{, see Figure~\ref{fig:ros}a}. On one core of an Intel
...
\red{A subset of 14 proteins of the PSICOV dataset, see
...
constraints resulting in higher model qualities. \red{ This
...
\caption{Folding performance on the full PSICOV dataset. (a) The 
 ...
 contact per residue on average. \red{Error bars indicate
...
\red{In addition, improvements in contact prediction methods further increase the quality
...
hardest to fold, see Figure~\ref{fig:main}b \red{ and
...
TM-score. \red{This might be explained by higher
...
\red{The difference between two TM-score values is significant
...
the models generated by CNS.} \red{To provide a TM-score
...
TM-score of 0.34.} \red{For PconsFold and Rosetta/plmDCA the
...
\begin{table}[!t]
    \processtable{\red{Average TM-scores for top-ranked models.
...
for EVfold-PLM. \red{Additional analysis with PROCHECK confirmed this
...
field \red{within the 90\% significance interval}, showing the
...
of 0.64 \red{within the 95\% significance interval}. This
further supports our observation that improved contact maps
improve structure prediction.

\red{Generally, co-evolution based methods are applicable to larger proteins, as
...
Mean & 0.48 & 0.55 \red{(0.09$^\dagger$)} & 0.64 \red{(0.04$^\ddagger$)} \\ \botrule
...
possible model. \red{ $\dagger$ p-value for a one-sample t-test of TM-score
...
quality models. \red{According to
...
\red{Our results on different fold classes show that model
